...
export default {
  ...
  components: {
    formEdit: () => import('./form/Webinar'),
    formAdd: () => import('./form/Webinar')
  },
  ...
  methods: {
    ...
    loadTable() {
      axios
        .get(`http://localhost:3001/webinar?page=${this.page}&limit=${this.limit}`)
        .then(res => {
    ...
    deleteItem(id) {
      this.showMsgDialog(
        ...
        if (res.value) {
          axios
            .delete(`http://localhost:3001/webinar/${id}`)
