            ...
            <v-card tile elevation="1" text class="skemaTableDiv">
              ...
              >
                <v-toolbar-title
                  ...
                  "
                >
                  Daftar Pemegang Sertifikat LSP Promigas Indonesia
                </v-toolbar-title>
                <v-btn
              ...
              >
                ...
                <template v-slot:[`header.nama_asesi`]="{ header }">
                  <div style="font-weight:bold; padding:0;font-size:14px">
                    {{ header.text }}
                  </div>
                </template>
                <template v-slot:[`header.sertifikat_kompetensi`]="{ header }">
                  <div style="font-weight:bold; padding:0;font-size:14px">
                    {{ header.text }}
                  </div>
                </template>
                <template v-slot:[`header.tanggal_terbit`]="{ header }">
                  <div style="font-weight:bold; padding:0;font-size:14px">
                    {{ header.text }}
                  </div>
                </template>
                ...
                <template v-slot:[`item.nama_asesi`]="{ item }">
                  <div v-html="item.nama_asesi"></div>
                </template>
                <template v-slot:[`item.tanggal_terbit`]="{ item }">
                  <td>
                    {{ convertDate(item.tanggal_terbit) }}
                  </td>
                </template>
...
export default {
  ...
  components: {
    formEdit: () => import('./form/Pemegang_Sertifikat'),
    formAdd: () => import('./form/Pemegang_Sertifikat')
  },
  ...
  methods: {
    ...
    addItem() {
      setTimeout(() => {
        document.getElementById('pemegangSertifikatAdd').click()
      }, 500)
    },
    editItem(val) {
      this.detail = val
      setTimeout(() => {
        document.getElementById('pemegangSertifikatEdit').click()
      }, 500)
    },
    deleteItem(id) {
      this.showMsgDialog(
