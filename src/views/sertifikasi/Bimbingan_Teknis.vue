            ...
            <v-card tile elevation="1" text class="skemaTableDiv">
              ...
              >
                <v-toolbar-title
                  ...
                  "
                >
                  Daftar Bimbingan Teknis LPK KPMI
                </v-toolbar-title>
                <v-btn
              ...
              >
                ...
                <template v-slot:[`header.nama`]="{ header }">
                  <div style="font-weight:bold; padding:0;font-size:14px">
                    {{ header.text }}
                  </div>
                </template>
                <template v-slot:[`header.tanggal`]="{ header }">
                  <div style="font-weight:bold; padding:0;font-size:14px">
                    {{ header.text }}
                  </div>
                </template>
...
export default {
  ...
  components: {
    formEdit: () => import('./form/Bimtek'),
    formAdd: () => import('./form/Bimtek')
  },

  data() {
    return {
      API_LINK: buildType.apiURL('development'),
      page: 1,
      limit: 10,
      totalPage: 0,
      interval: {},
  ...
  methods: {
    ...
    loadTable() {
      axios
        .get(`http://localhost:3001/bimtek?page=${this.page}&limit=${this.limit}`)
        .then(res => {
    ...
    addItem() {
      setTimeout(() => {
        document.getElementById('asesorAdd').click()
      }, 500)
    },
    editItem(val) {
      this.detail = val
      setTimeout(() => {
        document.getElementById('asesorEdit').click()
      }, 500)
    },
    deleteItem(id) {
      this.showMsgDialog(
        ...
        if (res.value) {
          axios
            .delete(`http://localhost:3001/bimtek/${id}`)
