<template>
  <v-container>
    <v-row>
      ...
      <v-col cols="12" sm="1" md="2">
        <v-text-field v-model="keyword" label="Keyword" prepend-icon="" />
      </v-col>
      <v-col cols="12" sm="3" md="1">
        <v-btn color="blue" class="ma-2 white--text" @click="submitSearch">
          Search
          <v-icon right dark>
            mdi-magnify
          </v-icon>
...
export default {
  ...
  created() {
    ...
    if (xLocalStorage.sanqua_company_id !== 0) {
      this.company = {
        id: xLocalStorage.sanqua_company_id,
        name: xLocalStorage.sanqua_company_name
      }
      this.items = [
        {
          id: xLocalStorage.sanqua_company_id,
